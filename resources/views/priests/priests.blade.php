...
@section('content')
    <div class="my-5 mx-auto max-width-980">
        <div class="my-4 text-2xl text-chocolate-500">Священнослужители</div>
        <nav>
                ...
                <ul class="pagination no-select">
                    ...
                    @foreach($abc1 as $value)
                        <li class="page-item" data-mdb-ripple-init>
                            <a class="page-link text-base" href="/priests/?abc={{$value}}">{{$value}}</a>
                        </li>
                    @endforeach
                </ul>
                <ul class="pagination no-select">
        ...
        <div class="my-3">
            <form action="/priests" class="input-group">
                <input type="search" name="search" id="search"
                       class="px-4 py-2 form-control rounded-start-5 border text-base text-chocolate-500"
                       placeholder="Поиск священнослужителя" value="{{ request('search') }}" autofocus/>
