...
@section('admin-content')
    <div class="mx-auto max-width-980 my-5 p-4 rounded-4 shadow-sm bg-chocolate-200">
        <h1 class="text-3xl mb-4 text-green-300">Редактирование старосты/ктитора</h1>
                ...
                <form action="/admin/elder-founders/{{ $elder_founder->slug }}" method="POST"
                    ...
                    <div class="max-w-lg">
                        ...
                        <div class="image-group">
                            <input name="old-image" type="hidden" value="{{ $elder_founder->image }}">

                            @if($elder_founder->image)
                                <img id="preview-image" src="{{ asset('storage/' . $elder_founder->image) }}"
                                     class="rounded-2 img-fluid border overflow-hidden" alt="">
                            @else
                                <img id="preview-image" src="" class="rounded-2 img-fluid border display-none" alt="">
                            @endif
                    ...
                    <div class="">
                        <div class="mb-2">
                            ...
                            <input id="firstname" name="firstname" type="text" autocomplete="firstname"
                                   ...
                                   value="{{ old('firstname', $elder_founder->firstname) }}" autofocus>
                            @error('firstname')
                        ...
                        <div class="mb-2">
                            ...
                            <input id="secondname" name="secondname" type="text" autocomplete="secondname"
                                   class="container-fluid px-4 py-2 block rounded-pill
                                          text-base border text-chocolate-500 border-chocolate-300 bg-chocolate-100"
                                   value="{{ old('secondname', $elder_founder->secondname) }}" autofocus>
                            @error('secondname')
                            <small class="my-2 block text-base text-red-600">{{ $message }} </small>
                            @enderror
                        </div>
                        <div class="mb-2">
                            ...
                            <input id="slug" name="slug" type="text" readonly autocomplete="slug"
                                   ...
                                   value="{{ old('slug', $elder_founder->slug) }}">
                            @error('slug')
                    ...
                    <div class="">
                        ...
                        <textarea id="description" type="hidden" name="biography"
                                  value="{{ old('biography', $elder_founder->biography) }}">
                             {{ $elder_founder->biography }}
                        </textarea>
                    ...
                    <div class="my-4 input-group items-right-2">
                         ...
                         <button type="submit" class="m-1 px-4 btn btn-outline-success rounded-pill save-icon text-base">&nbsp; Сохранить</button>
                    </div>
