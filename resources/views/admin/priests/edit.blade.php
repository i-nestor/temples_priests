...
@section('admin-content')
    <div class="mx-auto max-width-980 my-5 p-4 rounded-4 shadow-sm bg-chocolate-200">
        <h1 class="text-3xl mb-4 text-green-600">Редактирование священнослужителя</h1>
                ...
                <form action="/admin/priests/{{ $priest->slug }}" method="POST"
                    ...
                    <div class="max-w-lg">
                        ...
                        <div class="image-group">
                            <input name="old-image" type="hidden" value="{{ $priest->image }}">

                            @if($priest->image)
                                <img id="preview-image" src="{{ asset('storage/' . $priest->image) }}"
                    ...
                    <div class="">
                        <div class="mb-2">
                            ...
                            <input id="firstname" name="firstname" type="text" autocomplete="firstname"
                                   ...
                                   value="{{ old('firstname', $priest->firstname) }}" autofocus>
                            @error('firstname')
                        ...
                        <div class="mb-2">
                            ...
                            <input id="secondname" name="secondname" type="text" autocomplete="secondname"
                                   class="container-fluid px-4 py-2 block rounded-pill
                                          text-base border text-chocolate-500 border-chocolate-300 bg-chocolate-100"
                                   value="{{ old('secondname', $priest->secondname) }}" autofocus>
                            @error('secondname')
                            <small class="my-2 block text-base text-red-600">{{ $message }} </small>
                            @enderror
                        </div>
                    ...
                    <div class="">
                        ...
                        <textarea id="description" type="hidden" name="biography"
                                  value="{{ old('biography', $priest->biography) }}">
                             {{ $priest->biography }}
                        </textarea>
    ...
    <script> {{-- Для автоматического заплнения slug --}}
        ...
        name.addEventListener('change', async function () {
            const res = await fetch(`/admin/priests/slug?${
